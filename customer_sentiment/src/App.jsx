...
function SectionCard({ title, subtitle, children, right, compact = false, className = "" }) {
  return (
    <section
      style={{ borderColor: palette.border, backgroundColor: palette.cardBg }}
      className={`rounded-2xl border shadow-[0_4px_14px_rgba(46,42,40,0.06)] ${
...
function KpiCard({
  ...
  const countedValue = useCountUp(valueMetric ? valueMetric.target : 0, 4000, valueMetric ? valueMetric.decimals : 0);
  const countedChange = useCountUp(changeMetric ? changeMetric.target : 0, 4000, changeMetric ? changeMetric.decimals : 0);
  ...
  return (
    <div
      className={`rounded-xl border p-2.5 shadow-[0_2px_10px_rgba(46,42,40,0.05)] ${
        centered ? "flex h-[210px] w-[360px] max-w-full items-center justify-center text-center" : inlineMetric ? "w-fit min-w-[230px]" : ""
      }`}
      style={{ borderColor: palette.border, backgroundColor: palette.cardBg }}
    >
      <div className={centered ? "flex flex-col items-center justify-center gap-2" : "flex items-start justify-between gap-2"}>
        <div className={centered ? "flex flex-col items-center" : "min-w-0"}>
          {inlineMetric ? (
            <p className="whitespace-nowrap text-xl font-semibold tracking-tight tabular-nums" style={{ color: palette.darkText }}>
            ...
            <>
              <p className="text-[9px] font-semibold uppercase tracking-[0.14em]" style={{ color: palette.mutedText }}>{title}</p>
              <p className="mt-0.5 text-xl font-semibold tracking-tight tabular-nums" style={{ color: palette.darkText }}>{displayValue}</p>
            </>
          )}

          <div className="mt-1 inline-flex items-center rounded-full px-1.5 py-0.5 text-[10px] font-medium tabular-nums" style={{ backgroundColor: accent.bg, color: accent.text }}>
            {displayChange}
          </div>
        </div>

        <div className={centered ? "flex h-8 w-8 items-center justify-center rounded-lg text-[10px] font-semibold" : "flex h-7 w-7 items-center justify-center rounded-lg text-[10px] font-semibold flex-shrink-0"} style={{ backgroundColor: accent.soft, color: accent.text }}>
          {iconLabel}
        </div>
      </div>
    </div>
...
function DoughnutChart({ data, centerLabel = "", size = 96 }) {
  ...
  const hole = Math.round(size * 0.50);
  return (
    <div className="relative flex-shrink-0" style={{ width: size, height: size }}>
      ...
      <div className="absolute left-1/2 top-1/2 flex items-center justify-center rounded-full border"
        ...
        <span className="text-center px-1 text-[7px] font-semibold uppercase leading-tight tracking-wide"
...
function PieLegend({ data, compact = false }) {
  return (
    <div className="flex flex-col" style={{ gap: 4 }}>
      {data.map((item) => (
        <div key={item.name} className={`flex items-center rounded-lg border px-2 py-1 ${compact ? "w-fit" : "justify-between"}`} style={{ borderColor: palette.neutralLight, backgroundColor: palette.cardBg }}>
          ...
          {!compact && <span className="ml-2 text-[11px] font-semibold" style={{ color: palette.darkText }}>{item.value}%</span>}
        </div>
      ))}
    </div>
  );
...
function ChartWithLegend({ title, subtitle, data, centerLabel, compact = false }) {
  return (
    <SectionCard title={title} subtitle={subtitle} compact={compact}>
      <div className={`items-center gap-2 ${compact ? "inline-flex w-fit max-w-full" : "flex"}`}>
        <DoughnutChart data={data} centerLabel={centerLabel} size={96} />
        <div className={compact ? "w-fit" : "flex-1 min-w-0"}>
...
function LoyaltyChartCard({ title, subtitle, data, centerLabel }) {
  return (
    <div className="flex h-[210px] w-[360px] max-w-full flex-col items-center justify-center rounded-xl border p-3 text-center shadow-[0_2px_10px_rgba(46,42,40,0.05)]" style={{ borderColor: palette.border, backgroundColor: palette.cardBg }}>
      <div className="mb-2">
        <h3 className="text-[13px] font-semibold leading-tight tracking-tight" style={{ color: palette.darkText }}>{title}</h3>
        {subtitle && <p className="mt-0.5 text-[10px] leading-3.5" style={{ color: palette.mutedText }}>{subtitle}</p>}
      </div>
...
// ─── Animated Polyline ────────────────────────────────────────────────────────
function AnimatedPolyline({ points, color, delay = 0 }) {
...
// ─── Mini Line Chart// ─── Mini Line Chart — compact 100px height ───────────────────────────────────
function MiniLineChart({ data, lines }) {
  const W = 640, H = 100, PX = 20, PT = 8, PB = 18;
  ...
  return (
    <div className="overflow-x-auto">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full min-w-[360px]" style={{ height: 100 }}>
        {[0, 1, 2].map((i) => {
        ...
        {lines.map((line, li) => {
          ...
          return (
            <g key={line.key}>
              <AnimatedPolyline points={pts} color={line.color} delay={li * 500} />
              {data.map((r, i) => (
...
function OverviewPage() {
  return (
    <div className="space-y-3">
      <div>
        <h2 className="text-xl font-semibold tracking-tight" style={{ color: palette.darkText }}>Overall Sentiment Overview</h2>
        <p className="text-[11px] leading-4 mt-0.5" style={{ color: palette.mutedText }}>
          Vertically stacked summary of sentiment, satisfaction, recommendation intent, and trend movement.
        </p>
      </div>

      <div className="mx-auto grid w-full max-w-xl grid-cols-2 gap-2">
        <KpiCard animate title="Total Feedback" value="12,458" change="+8.4% vs last 7 days" iconLabel="TF"
          accent={{ bg: palette.sectionTint, soft: palette.sectionTint, text: palette.primary }} />
        <KpiCard animate title="Positive Share" value="64.2%" change="+6.7% vs last 7 days" iconLabel="PS"
          accent={{ bg: palette.positiveLight, soft: palette.positiveLight, text: palette.positive }} />
        <KpiCard animate title="Negative Share" value="21.3%" change="+2.1% vs last 7 days" iconLabel="NS"
          accent={{ bg: palette.negativeLight, soft: palette.negativeLight, text: palette.negative }} />
        <KpiCard animate title="Recommendation" value="72.1%" change="+5.3% vs last 7 days" iconLabel="RR"
          accent={{ bg: palette.sectionTint, soft: palette.sectionTint, text: palette.primary }} />
      </div>
      ...
      <div className="mx-auto w-full max-w-xl">
        <ChartWithLegend title="Feedback Sentiment Distribution"
          ...
          data={sentimentDistribution} centerLabel="Sentiment" />
      </div>

      <div className="mx-auto w-full max-w-xl">
        <SectionCard title="Sentiment Trend" subtitle="Compact trend view below the sentiment split.">
      ...
      <div className="mx-auto w-full max-w-xl">
        <ChartWithLegend title="Customer Satisfaction Overview"
          ...
          data={satisfactionData} centerLabel="Satisfaction" />
      </div>

      <div className="mx-auto w-full max-w-xl">
        <ChartWithLegend title="Recommendation Intent Analysis"
          subtitle="How many people would recommend the service to others."
          data={recommendData} centerLabel="Recommend" />
      </div>
    </div>
  );
}

function NegativeInsightsPage() {
  ...
  return (
    <div className="space-y-3">
      <div>
        ...
        <p className="text-[11px] leading-4 mt-0.5" style={{ color: palette.mutedText }}>Top drivers, trend movement, search, and early warning signals.</p>
      </div>

      <SectionCard title="Search Negative Feedback Causes" subtitle="Search for the main cause of negative feedback.">
        <div className="relative">
          ...
          <input
            ...
            className="w-full rounded-lg border py-1.5 pl-7 pr-2.5 text-xs outline-none"
      ...
      <div className="flex flex-col items-start gap-3 lg:flex-row">
        <div className="w-full lg:w-1/2">
          <SectionCard title="Top 5 Negative Feedback Drivers" subtitle="Most frequent negative feedback themes.">
            <HorizontalBarChart data={filtered} color={palette.primary} />
          </SectionCard>
        </div>

        <div
          className="alert-glow w-full rounded-xl border p-2.5 lg:w-[20%]"
        ...
        >
          <div className="flex items-start gap-2">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[10px] leading-3.5 mt-0.5" style={{ color: palette.bodyText }}>
      ...
      <div className="mx-auto w-full max-w-xl">
        <SectionCard title="Top 5 Negative Feedback Trends" subtitle="Trend lines draw from left to right in 4 seconds.">
          <MiniLineChart data={negativeTrends} lines={[
...
function AIInsightsPage() {
  return (
    <div className="space-y-3">
      <div>
        <h2 className="text-xl font-semibold tracking-tight" style={{ color: palette.darkText }}>AI Insights from User Comments</h2>
        <p className="text-[11px] leading-4 mt-0.5" style={{ color: palette.mutedText }}>Real comments, an AI-written summary, and practical improvement suggestions.</p>
      </div>

      <div className="mx-auto w-full max-w-xl">
        <SectionCard title="Top User Comments" subtitle="Five representative comments surfaced for the user.">
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {comments.map((item, i) => (
              <div key={i}
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] leading-4 mt-0.5" style={{ color: palette.bodyText }}>{item.comment}</p>
                  </div>
                </div>
      ...
      <div className="mx-auto w-full max-w-xl">
        <SectionCard title="AI Summary of User Problems" subtitle="A concise summary of what users are facing and how to improve it.">
          <div className="grid gap-2 xl:grid-cols-[1.25fr_0.75fr]">
            <div className="rounded-xl border p-2.5" style={{ backgroundColor: palette.insightLight, borderColor: palette.border }}>
              <div className="mb-1.5 flex items-center gap-2">
                <div className="flex h-7 w-7 items-center justify-center rounded-lg text-[10px] font-semibold flex-shrink-0"
                ...
                <div>
                  ...
                  <p className="text-[10px] leading-3.5" style={{ color: palette.bodyText }}>What the users' problem looks like at a glance.</p>
                </div>
              </div>
              <p className="text-[11px] leading-4" style={{ color: palette.bodyText }}>
            ...
            <div className="rounded-xl border p-2.5" style={{ backgroundColor: palette.sectionTint, borderColor: palette.border }}>
              <p className="text-[9px] font-semibold uppercase tracking-[0.14em]" style={{ color: palette.mutedText }}>Primary Issue Impact</p>
              <p className="mt-1 text-2xl font-semibold" style={{ color: palette.primary }}>High</p>
              <p className="mt-1 text-[11px] leading-4" style={{ color: palette.bodyText }}>
      ...
      <SectionCard title="How to Improve" subtitle="Suggested actions for quick decision-making.">
        <div className="grid gap-1.5 md:grid-cols-2">
          {[
            ...
            <div key={item} className="flex items-start gap-1.5 rounded-lg border px-2.5 py-2"
              style={{ borderColor: palette.neutralLight, backgroundColor: palette.cardBg }}>
              <div className="mt-0.5 flex h-4 w-4 items-center justify-center rounded-full text-[9px] font-semibold flex-shrink-0"
                style={{ backgroundColor: palette.softHighlight, color: palette.primary }}>✓</div>
              <p className="text-[11px] leading-4" style={{ color: palette.bodyText }}>{item}</p>
            </div>
          ))}
        </div>
      </SectionCard>
...
function LoyaltyPage() {
  return (
    <div className="space-y-3">
      ...
      <div className="mx-auto grid w-fit grid-cols-1 place-items-center gap-3 md:grid-cols-2">
        ...
        <KpiCard animate inlineMetric centered title="Satisfaction Rate" value="71.2%" change="+6.5% vs last 7 days" iconLabel="SR" accent={{ bg: palette.sectionTint, soft: palette.sectionTint, text: palette.primary }} />
        <KpiCard animate inlineMetric centered title="Net Promoter Score" value="+34" change="+7 points vs last 7 days" iconLabel="NP" accent={{ bg: palette.softHighlight, soft: palette.softHighlight, text: palette.primary }} />
      </div>

      <div className="mx-auto w-full max-w-xl">
        <SectionCard title="Customer Loyalty Insight" subtitle="A plain-language summary for non-technical viewers.">
          <div className="rounded-xl border p-2.5" style={{ backgroundColor: palette.sectionTint, borderColor: palette.border }}>
            <p className="text-[11px] leading-4" style={{ color: palette.bodyText }}>
              Overall sentiment remains healthy, with most users satisfied and willing to recommend the service.
              However, recurring support delays and billing issues are reducing loyalty among a smaller but important
              group of users. Solving those two problems is likely to improve both satisfaction and advocacy.
            </p>
          </div>
        </SectionCard>
      </div>
...
export default function SentimentDashboardVertical() {
  ...
  return (
    ...
    <div className="min-h-screen w-full px-4 py-4 md:px-6" style={{ backgroundColor: palette.pageBg }}>
      <div className="relative mx-auto max-w-5xl space-y-3">
          ...
          <div className="flex flex-col gap-2 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex items-start gap-2.5">
              ...
              <div>
                ...
                <p className="text-[11px] leading-4" style={{ color: palette.mutedText }}>
                  A clean dashboard with compact chart layouts and concise insight blocks.
                </p>
              </div>
            </div>
            <div className="rounded-xl border px-2.5 py-1.5 flex-shrink-0"
              ...
              <p className="text-[11px] font-medium mt-0.5" style={{ color: palette.darkText }}>May 12 – May 18, 2024</p>
            </div>
          </div>

          <div className="mt-2 grid gap-1.5 md:grid-cols-3">
            {[
              { label: "Most Critical Issue",  value: "Support Delay"      },
              { label: "Recommendation Trend", value: "Up 5.3% this week" },
              { label: "AI Summary Status",    value: "Updated 2 min ago" },
            ].map(({ label, value }) => (
              <div key={label} className="rounded-xl border px-2.5 py-1.5"
                style={{ borderColor: palette.border, backgroundColor: palette.sectionTint }}>
                <p className="text-[9px] font-semibold uppercase tracking-[0.14em]" style={{ color: palette.mutedText }}>{label}</p>
                <p className="text-[11px] font-semibold mt-0.5" style={{ color: palette.darkText }}>{value}</p>
              </div>
            ))}
          </div>
        ...
        <div className="relative">
          ...
          {activeTab === "Negative Insights" && <NegativeInsightsPage />}
          {activeTab === "AI Insights"       && <AIInsightsPage />}
          {activeTab === "Customer Loyalty"  && <LoyaltyPage />}
        </div>
      </div>
      </div>
    </>
